...
function Routes() {
  ...
  const inputStyle = { background: '#1e293b', border: '1px solid #334155', color: '#94a3b8', borderRadius: '10px', padding: '9px 14px', fontSize: '13px', outline: 'none' }
    ...
    <div style={{minHeight: '100vh', background: '#0f172a'}}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div style={{marginBottom: '28px'}}>
          ...
          <p style={{color: '#64748b', fontSize: '14px', margin: 0}}>{total} rutas disponibles</p>
        </div>

        <div style={{background: '#1e293b', border: '1px solid #334155', borderRadius: '14px', padding: '16px 20px', marginBottom: '24px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center'}}>
          <input
            type="text"
        ...
        {loading && <div style={{color: '#94a3b8', textAlign: 'center', padding: '48px'}}>Cargando rutas...</div>}
        ...
        {!loading && routes.length === 0 && (
          <div style={{textAlign: 'center', padding: '64px 0'}}>
            <p style={{color: '#475569', fontSize: '18px', marginBottom: '8px'}}>No hay rutas con esos filtros</p>
            <p style={{color: '#334155', fontSize: '14px'}}>Intenta cambiar los filtros o publica la primera</p>
          </div>
        )}
        ...
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px', marginBottom: '28px'}}>
          {routes.map((route) => (
            <a key={route.id} href={"/routes/" + route.id} style={{textDecoration: 'none', display: 'block'}}>
              <div style={{background: '#1e293b', border: '1px solid #334155', borderRadius: '14px', overflow: 'hidden', transition: 'border-color 0.2s'}}
              ...
              >
                {route.photos && route.photos.length > 0 ? (
                  ...
                  <div style={{width: '100%', height: '160px', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                    <span style={{color: '#334155', fontSize: '40px'}}>?</span>
                  </div>
                )}
                <div style={{padding: '16px'}}>
                  ...
                  <p style={{color: '#64748b', fontSize: '13px', margin: '0 0 12px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden'}}>{route.description}</p>
                  <div style={{display: 'flex', gap: '12px', fontSize: '12px', color: '#475569', marginBottom: '12px'}}>
                    <span>{route.distanceKm} km</span>
                    {route.elevationM && <span>{route.elevationM} m</span>}
                    {route.estimatedTime && <span>{route.estimatedTime} min</span>}
                  </div>
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '10px', borderTop: '1px solid #0f172a'}}>
                    <span style={{color: '#475569', fontSize: '12px'}}>por <span style={{color: '#ec4899'}}>{route.user.username}</span></span>
        ...
        {totalPages > 1 && (
          <div style={{display: 'flex', justifyContent: 'center', gap: '8px'}}>
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} style={{background: '#1e293b', color: page === 1 ? '#475569' : '#94a3b8', border: '1px solid #334155', borderRadius: '8px', padding: '8px 16px', fontSize: '13px', cursor: page === 1 ? 'not-allowed' : 'pointer'}}>
              Anterior
            </button>
            <span style={{color: '#64748b', fontSize: '13px', padding: '8px 12px'}}>
              {page} de {totalPages}
            </span>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} style={{background: '#1e293b', color: page === totalPages ? '#475569' : '#94a3b8', border: '1px solid #334155', borderRadius: '8px', padding: '8px 16px', fontSize: '13px', cursor: page === totalPages ? 'not-allowed' : 'pointer'}}>
              Siguiente
            </button>
          </div>
        )}
