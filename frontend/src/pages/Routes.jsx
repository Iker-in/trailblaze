...
function Routes() {
  ...
  const loadRoutes = async () => {
    ...
    try {
      ...
      setRoutes((prev) => (page === 1 ? data.routes : [...prev, ...data.routes]))
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {loading && (
          <div style={{display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px", marginBottom: "28px"}}>
            {[0, 1, 2, 3, 4, 5].map((index) => <RouteCardSkeleton key={index} />)}
          </div>
        )}
        ...
        {!loading && !hasMore && routes.length > 0 && (
          <p style={{textAlign: "center", color: "#1A3050", fontSize: "13px", padding: "32px 0"}}>Has visto todas las rutas</p>
        )}
